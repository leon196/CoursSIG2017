...
public static class Utils
{
	...
	public static void MapVertexUV (Mesh[] meshArray, int resolution, int stride = 1)
	{
		int vertexIndex = 0;
		foreach (Mesh mesh in meshArray) {
			Vector2[] uvs2 = new Vector2[mesh.vertices.Length];
			for (int i = 0; i < uvs2.Length/stride; i += stride) {
				float x = vertexIndex % resolution;
				float y = Mathf.Floor(vertexIndex / (float)resolution);
				for (int s = 0; s < stride; ++s) {
					uvs2[i*stride+s] = new Vector2(x, y) / (float)resolution;
					++vertexIndex;
				}
			}
			mesh.uv2 = uvs2;
